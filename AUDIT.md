...
**Files:** `index.php`, `midjourney.php`, `cinematic.php`, `_index.html`
**Status:** Fixed
...
**Files:** `index.php`, `midjourney.php`
**Status:** Fixed (April 15, 2026)
...
`index.php` and `midjourney.php` already use shared CSS files. `cinematic.php` was previously styled separately and is now being aligned by loading shared + module CSS assets. Styles still diverge in several component blocks, so global changes can require touching multiple files.
...
`cinematic.php` is now wired to load shared + module CSS files directly.
...
### ARCH-03 — Shared JS extraction is incomplete
The project now loads external JS modules, but module-specific logic (`buildPrompt`, search/filter behavior, stats, and some UI handlers) is still duplicated between builders. Bug fixes still require touching multiple files.
...
| BUG-07 | Done tab non-functional | index.php + script.js | Added handlers + setFilter logic |
| BUG-08 | Nav action buttons were stubs | index.php + midjourney.php + core.js | Added handlers and feedback actions |
| BUG-09 | Missing aria labels on arrows | index.php + midjourney.php | Added aria-label attributes |
